...
class Stadium extends Component {
    ...
    renderStadium() {
        ...
        for (let i = 1; i < this.state.team.goal + 1; i++) {
            span.push(<button onClick={() => this.props.onCircleSelect("goal")} className="goal" key={"goal"}
                              value={"goal"}><span>{this.props.data["goal"] ? this.props.data["goal"] : ""}</span></button>);
        }
        for (let i = 1; i < this.state.team.defenser + 1; i++) {
            span.push(<button onClick={() => this.props.onCircleSelect("defenser" + i)} className="defenser" key={"defenser" + i}
        ...
        for (let i = 1; i < this.state.team.middle + 1; i++) {
            span.push(<button onClick={() => this.props.onCircleSelect("middle" + i)} className="middle" key={"middle" + i}
        ...
        for (let i = 1; i < this.state.team.attacker + 1; i++) {
            span.push(<button onClick={() => this.props.onCircleSelect("attacker" + i)} className="attacker" key={"attacker" + i}
